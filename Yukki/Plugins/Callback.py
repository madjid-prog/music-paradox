...
async def admin_risghts(_, CallbackQuery):
    ...
    if command == "pausecb":
        ...
        await CallbackQuery.message.reply_text(
            f"🎧 Obrolan Suara Dijeda oleh {CallbackQuery.from_user.mention}!",
    ...
    if command == "resumecb":
        ...
        await CallbackQuery.message.reply_text(
            f"🎧 Obrolan Suara Dilanjutkan oleh {CallbackQuery.from_user.mention}!",
            reply_markup=audio_markup2,
        )
        await CallbackQuery.message.delete()
    ...
    if command == "stopcb":
        ...
        await CallbackQuery.message.reply_text(
            f"🎧 Obrolan Suara Berakhir/Dihentikan oleh {CallbackQuery.from_user.mention}!",
            reply_markup=audio_markup2,
        )
        await CallbackQuery.message.delete()
...
async def play_playlist(_, CallbackQuery):
    ...
    if smex == "third":
        ...
        if CallbackQuery.from_user.id != int(user_id):
            return await CallbackQuery.answer(
                "Ini bukan untukmu! Mainkan daftar putar Anda sendiri", show_alert=True
            )
        _playlist = await get_playlist_names(user_id, type)
    ...
    try:
        ...
        if read1:
            return await CallbackQuery.answer(
                "Pemutaran Streaming Langsung... Hentikan untuk memutar daftar putar",
                show_alert=True,
            )
        else:
            pass
    ...
    if not _playlist:
        ...
        await CallbackQuery.message.delete()
        mystic = await CallbackQuery.message.reply_text(
            f"Memulai Daftar Putar Dari {third_name}.\n\nDi req oleh:- {CallbackQuery.from_user.first_name}"
        )
        ...
        for shikhar in _playlist:
            ...
            if await is_active_chat(chat_id):
                ...
                cap = f"🎥<b>__Memutar:__ </b>[{title[:25]}](https://www.youtube.com/watch?v={videoid}) \n💡<b>__Info:__</b> [Dapatkan Informasi Tambahan](https://t.me/{BOT_USERNAME}?start=info_{videoid})\n👤**__Di req oleh:__** {CallbackQuery.from_user.mention}"
                final_output = await CallbackQuery.message.reply_photo(
                    photo=thumb,
                    reply_markup=InlineKeyboardMarkup(buttons),
                    caption=cap,
        ...
        if for_p == 1:
            m = await CallbackQuery.message.reply_text(
                "Menempel Antrian Daftar Putar ke Bin"
            )
            ...
            await CallbackQuery.message.reply_text(
                "Hanya 1 Musik di Daftar Putar.. Tidak ada lagi musik untuk ditambahkan dalam antrean."
            )
        if for_t == 1:
            await start_timer(
                send_video,
...
async def group_playlist(_, CallbackQuery):
    ...
    if not _count:
        sex = await CallbackQuery.message.reply_text(
            f"Selamat Datang di {MUSIC_BOT_NAME}'s Fitur Daftar Putar.\n\nMembuat Daftar Putar Anda di Basis Data... Harap tunggu.\n\nGenre:- {genre}"
        )
    ...
    return await CallbackQuery.message.reply_text(
        f"Ditambahkan ke {type}'s {genre} Daftar putar oleh {CallbackQuery.from_user.mention}"
    )
...
async def check_playlist(_, CallbackQuery):
    ...
    if not _playlist:
        return await CallbackQuery.answer(
            f"Tidak ada {genre} Daftar putar di server. Coba tambahkan musik di daftar putar.",
            show_alert=True,
        )
    else:
        j = 0
        ...
        if await isPreviewUp(preview):
            await CallbackQuery.message.reply_photo(
                photo=preview,
                caption=f"Ini adalah Daftar Putar dari {user_name}.",
                quote=False,
                reply_markup=InlineKeyboardMarkup(buttons),
            )
            await m.delete()
...
async def del_playlist(_, CallbackQuery):
    ...
    if not _playlist:
        return await CallbackQuery.answer(
            "Grup tidak memiliki Daftar Putar di Server Bot", show_alert=True
        )
    else:
        await CallbackQuery.message.delete()
        await CallbackQuery.answer()
    ...
    await CallbackQuery.message.reply_text(
        f"Berhasil Menghapus {type}'s {genre} seluruh daftar putar\n\noleh :- {CallbackQuery.from_user.mention}"
    )
